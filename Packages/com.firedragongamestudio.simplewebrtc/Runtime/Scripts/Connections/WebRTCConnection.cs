...
namespace SimpleWebRTC {
    public class WebRTCConnection : MonoBehaviour {
        ...
        // ws://192.168.1.4:8765 windows
        // 192.168.1.4:8765 mac
        ...
        public static event System.Func<RenderTexture> OnRequestVideoTrack;
        ...
        // Handle alla coroutine WebRTC.Update() (pompa l'encoder nativo). Serve per fermarla
        // davvero: StopCoroutine(WebRTC.Update()) passa un IEnumerator NUOVO e diverso da quello
        // avviato, quindi non ferma nulla. Senza handle, ogni (ri)connessione accumulava una
        // coroutine in più che restava viva per sempre → con un video track attivo tutte pompano
        // lo stesso encoder insieme → contesa → freeze (tipico entrando in app col server spento,
        // dove ConnectClient ritenta in loop e accumula decine di coroutine).
        ...
        private void Update() {
            ...
            // Stop PRIMA di Start: Connect() setta entrambi i flag per "riavviare" la coroutine.
            // Con lo StopCoroutine ora funzionante (via handle), processare lo Start prima dello
            // Stop avviava e poi uccideva subito WebRTC.Update() nello stesso frame → encoder
            // fermo → nessun video. Nell'ordine giusto: Stop ferma la vecchia, Start avvia la nuova.
            StopWebRTCUpdate();
            StartWebRTCUpdate();
            CreateOffer();
            CreateAnswer();
            ...
            if (stopAllCoroutines) {
                ...
                StopAllCoroutines();
                // StopAllCoroutines ha già fermato anche WebRTC.Update(): azzera l'handle così
                // la prossima connessione la riavvia (altrimenti resterebbe "già avviata" ma morta).
        ...
        // Chiamato da WebRTCManager quando l'ultimo peer si disconnette (DISPOSE o PEERLEFT).
        // Resetta il flag così StartVideoTransmission() può essere chiamato di nuovo
        // quando il peer si riconnette, senza che il guard lo blocchi.
        public void ResetVideoTransmissionState() {
            // Libera il VideoStreamTrack oltre ai flag: senza Stop()+Dispose() il vecchio
            // track resta vivo e continua a catturare la RenderTexture. Alla riconnessione
            // ne verrebbe creato un secondo sulla stessa RT → contesa sull'encoder → freeze
            // al primo frame. Non usiamo StopVideoTransmission() perché fa SetActive su
            // StreamingCamera (qui null → NullReferenceException).
        ...
        public void StartVideoTransmission() {
            // Guard: evita di avviare una seconda trasmissione se una è già attiva.
            // Il flag viene resettato da ResetVideoTransmissionState() quando tutti
            // i peer si disconnettono, permettendo il riavvio dopo una riconnessione.
            if (IsVideoTransmissionActive) {
                Debug.LogWarning("[WebRTCConnection] StartVideoTransmission ignorato: trasmissione già attiva.");
                return;
            }
            // Imposta SUBITO il flag (non a fine coroutine): StartVideoTransmissionAsync
            // attende 1s prima di settarlo, quindi due chiamate ravvicinate (es. VideoManager
            // e WebRTCController sullo stesso bottone A) passavano entrambe il guard e
            // aggiungevano DUE track video → doppia codifica VP8 sul Quest → freeze.
        ...
        private IEnumerator StartVideoTransmissionAsync() {
            ...
            // Priorità sorgente video:
            // 1. ImmersiveSetup (cubemap equirect per VR 360°)
            // 2. OnRequestVideoTrack (hook custom, usato da VideoManager per la RenderTexture)
            // 3. StreamingCamera (camera Unity standard, fallback del package originale)
            if (UseImmersiveSetup) {
                videoStreamTrack = new VideoStreamTrack(videoEquirect);
            } else if (OnRequestVideoTrack != null) {
                var customRT = OnRequestVideoTrack.Invoke();
                videoStreamTrack = new VideoStreamTrack(customRT);
            } else if (StreamingCamera != null) {
                videoStreamTrack = StreamingCamera.CaptureStreamTrack(VideoResolution.x, VideoResolution.y);
            } else {
                Debug.LogError("[WebRTCConnection] Nessuna StreamingCamera e nessun OnRequestVideoTrack registrato.");
                IsVideoTransmissionActive = false; // reset: nessun track aggiunto, permetti retry
                yield break;
            }

            // Aggiunge il track a tutte le peer connection attive e triggera una
            // rinegoziazione (nuovo OFFER) per informare il receiver del nuovo track.
        ...
        private void StartWebRTCUpdate() {
            if (startWebRTCUpdate) {
                ...
                // Avvia SOLO se non è già in esecuzione: evita coroutine WebRTC.Update() duplicate
                // (ognuna pompa l'encoder → più copie insieme = freeze).
        ...
        private void StopWebRTCUpdate() {
            if (stopWebRTCUpdate) {
                ...
                // Ferma tramite l'handle salvato: StopCoroutine(WebRTC.Update()) NON funziona
                // perché genera un IEnumerator diverso da quello avviato.
                if (webRTCUpdateCoroutine != null) {
