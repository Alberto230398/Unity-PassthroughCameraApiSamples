...
namespace SimpleWebRTC {
    public class WebRTCManager {
        ...
        // Tenuto in memoria per aggiungerlo automaticamente alle nuove peer connection
        private VideoStreamTrack activeVideoStreamTrack;
        ...
        private void SetupPeerConnection(string peerId) {
            // Se esiste già un peer con questo ID è "stale": tipicamente il server è caduto e
            // tornato mentre lo stream era attivo, e il browser si riconnette con lo stesso ID
            // PRIMA che sia arrivato PEERLEFT/DISPOSE (il server era offline, non li ha inviati) o
            // che l'ICE andasse in Failed. Senza pulizia, peerConnections.Add() lancia
            // "An item with the same key has already been added". Rimuoviamo la vecchia peer
            // (chiude PC, data channel, sender e receiver) prima di ricrearla pulita.
            if (peerConnections.ContainsKey(peerId)) {
                RemovePeer(peerId, "STALE-RECONNECT");
            }
        ...
        private void HandleMessageInternal(byte[] bytes, int offset = 0, int length = 0) {
            ...
            switch (signalingMessage.Type) {
                ...
                case SignalingMessageType.DISPOSE:
                    // DISPOSE viene inviato dal peer che si disconnette volontariamente
                    // (es. click su "Disconnect"). Rimuoviamo tutte le risorse legate a quel peer.
                    RemovePeer(signalingMessage.SenderPeerId, "DISPOSE");
                    break;

                case SignalingMessageType.PEERLEFT:
                    // PEERLEFT viene inviato dal SERVER quando un peer perde la connessione
                    // WebSocket (es. chiude la tab, cade la rete) senza inviare DISPOSE.
                    // Dobbiamo pulire le stesse risorse di DISPOSE, altrimenti la vecchia
                    // peer connection resta aperta e quando il peer si riconnette con lo stesso
                    // ID, SetupPeerConnection va in eccezione perché la chiave esiste già.
                    RemovePeer(signalingMessage.SenderPeerId, "PEERLEFT");
                    break;
            ...
            foreach (var peerConnection in peerConnections) {
                ...
                foreach (var transceiver in transceivers) {
                    if (transceiver.Sender != null && transceiver.Sender?.Track?.Kind == TrackKind.Video) {
                        // VP8 + rtx/red/ulpfec: senza rtx una perdita pacchetti non viene
                        // recuperata (NACK senza retransmission) e il video resta freezato
                        // finché non arriva un nuovo keyframe → blocco dopo pochi secondi.
                        var caps = RTCRtpSender.GetCapabilities(TrackKind.Video).codecs;
                        var preferred = caps.Where(c =>
        ...
        // Centralizza la rimozione di un singolo peer (usato da DISPOSE e PEERLEFT).
        // Separato da CloseWebRTC() che rimuove TUTTI i peer contemporaneamente.
        private void RemovePeer(string peerId, string reason) {
            if (!peerConnections.ContainsKey(peerId)) return;

            peerConnections[peerId].Close();
            peerConnections.Remove(peerId);
            senderDataChannels.Remove(peerId);
            receiverDataChannels.Remove(peerId);
            videoTrackSenders.Remove(peerId);
            audioTrackSenders.Remove(peerId);

            if (VideoReceivers.ContainsKey(peerId))
                connectionGameObject.DestroyVideoReceiverGameObject(peerId, true);
            if (AudioReceivers.ContainsKey(peerId))
                connectionGameObject.DestroyAudioReceiverGameObject(peerId, true);

            // Se non rimane nessun peer attivo, permettiamo il riavvio della trasmissione video.
            // Questo sblocca il guard in StartVideoTransmission() dopo una riconnessione.
            if (videoTrackSenders.Count == 0)
                connectionGameObject.ResetVideoTransmissionState();

            SimpleWebRTCLogger.Log($"{reason}: rimosso peer {peerId} da {localPeerId}");
        }
        ...
        public void AddVideoTrack(VideoStreamTrack videoStreamTrack) {
            ...
            activeVideoStreamTrack = videoStreamTrack;
            ...
            foreach (var peerConnection in peerConnections) {
                videoTrackSenders[peerConnection.Key] = peerConnection.Value.AddTrack(videoStreamTrack);
            }
            connectionGameObject.CreateOfferCoroutine();
        }
