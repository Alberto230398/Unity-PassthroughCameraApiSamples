...
namespace SimpleWebRTC {
    ...
    public class SessionDescription : IJsonObject<SessionDescription> {
        ...
        public string StripNonVP8CodecsFromSdp() {
            ...
            // Step 1b: Keep rtx/red/ulpfec associated with VP8 (apt=) so retransmission
            // and FEC survive the filtering → recupero dalle perdite pacchetti.
            var associatedPt = new HashSet<string>();
            foreach (var line in lines) {
                var fmtp = Regex.Match(line, @"a=fmtp:(\d+)\s+apt=(\d+)");
                if (fmtp.Success && allowedPt.Contains(fmtp.Groups[2].Value))
                    associatedPt.Add(fmtp.Groups[1].Value);
            }
            allowedPt.UnionWith(associatedPt);
